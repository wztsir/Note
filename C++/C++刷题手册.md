...
完成的是文字接龙，输入一段文字，输出也是一段文字

##### 预训练 

自监督学习：自监督学习就是没有标签也要自己创建监督学习的条件，即当前只有样本x但是没有标签y ^ \hat{y} 
y
^

 ，那具体的做法就是将样本x分成两部分x ′ x\primex′和x ′ ′ x\prime \primex′′，其中一部分作为输入模型的样本，另一部分来作为标签：

在GPT中使用文本作为训练资料，切分前后文本，作为自监督模型的输入

##### 微调

在预训练的基础上微调模型，监督学习

微调非常有用，在预训练的基础上看过不同的语言的文本，在微调中面对实际的QA任务，只看过英文的训练资料也可以作出相应的输出

##### 强化学习

对于chatgpt给出的答案给出相应的评价

整个训练过程如下图所示：

![](.\image\2023-04-02 225112.png)
